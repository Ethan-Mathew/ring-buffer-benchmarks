...
template <typename T>
static void BM_SPSCBuffer(benchmark::State& state)
{
...
#define REGISTER_SPSC_BUFFER_BENCHMARK(PayloadType) \
    BENCHMARK_TEMPLATE(BM_SPSCBuffer, PayloadType)  \
        ->Arg(1)                                     \
        ->Arg(4)                                     \
        ->Arg(64)                                    \
        ->Arg(256)                                   \
...
REGISTER_SPSC_BUFFER_BENCHMARK(bench::data::BenchPayload<64>);
REGISTER_SPSC_BUFFER_BENCHMARK(bench::data::BenchPayload<256>);