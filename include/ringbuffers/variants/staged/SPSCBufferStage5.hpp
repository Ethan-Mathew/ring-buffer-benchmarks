/*
    ...
    + ALIGNED NON-ATOMIC CURSORS
    + BRANCH-BASED WRAPPING INDICES
*/
...
public:
    explicit SPSCBufferStage5(std::size_t bufferSize)
    {
        if (bufferSize < 1)
        {
            throw std::invalid_argument("SPSCBufferStage5 capacity must be greater than or equal to 1");
        }
    ...
    SPSCBufferStage5(const SPSCBufferStage5&) = delete;
    SPSCBufferStage5& operator=(const SPSCBufferStage5&) = delete;
    SPSCBufferStage5(SPSCBufferStage5&&) = delete;
    SPSCBufferStage5& operator=(SPSCBufferStage5&&) = delete;
    ...
    void pop()
    {
        ...
        popCursor_.store(popCursorNext, std::memory_order_release);
    }
...
private:
    ...
    alignas(cacheLineSize) std::size_t popCursorCache_ = 0;

    alignas(cacheLineSize) std::atomic<std::size_t> popCursor_ = 0;
    alignas(cacheLineSize) std::size_t pushCursorCache_ = 0;
